...
from .sql_updates import (check_already_followed, check_already_liked,
                          check_already_unfollowed,
                          check_and_insert_user_agent, check_and_update,
                          get_username_random, get_username_to_unfollow_random,
                          get_usernames, get_usernames_first, insert_media,
                          insert_unfollow_count, insert_username)
...
class InstaBot:
    ...
    def check_exisiting_comment(self, media_code):
        ...
        if check_comment.status_code == 200:
            ...
            insert_media(
                self, self.media_by_tag[0]["node"]["id"], str(check_comment.status_code)
            )
            ...
            if self.login_status == 1:
                ...
                try:
                    ...
                    self.media_on_feed = list(
                        all_data["graphql"]["user"]["edge_web_feed_timeline"]["edges"]
                    )
